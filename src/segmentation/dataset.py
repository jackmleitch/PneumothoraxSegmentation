...
from rle import run_length_decode
...
def get_transforms(phase, size, mean, std):
    ...
    list_transforms.extend(
        [albu.Normalize(mean=mean, std=std, p=1), albu.Resize(size, size), ToTensor(),]
    )
...
def provider(
    ...
    kfold = StratifiedKFold(total_folds, shuffle=True, random_state=42)
